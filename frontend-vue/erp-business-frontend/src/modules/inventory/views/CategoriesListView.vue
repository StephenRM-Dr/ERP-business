<script setup lang="ts">
import { computed, onMounted, ref, watch } from 'vue';
import { useI18n } from 'vue-i18n';

import ConfirmDialog from '@/components/ui/ConfirmDialog.vue';
import PaginationBar from '@/components/ui/PaginationBar.vue';
import SearchInput from '@/components/ui/SearchInput.vue';
import StatusChip from '@/components/ui/StatusChip.vue';
import { usePagination } from '@/composables/usePagination';
import type { Category } from '../interfaces/category.interface';
import { useCategoryStore } from '../interfaces/category.store';

const { t } = useI18n();
const categoryStore = useCategoryStore();

const searchTerm = ref<string>('');
const categoryToDelete = ref<Category | null>(null);
const isDeleting = ref<boolean>(false);

onMounted(() => {
  categoryStore.fetchCategories();
});

const filteredCategories = computed<Category[]>(() =>
  categoryStore.sortedCategories.filter((category) => {
    const term = searchTerm.value.trim().toLowerCase();
    return (
      term === '' ||
      category.nombre.toLowerCase().includes(term) ||
      category.codigo.toLowerCase().includes(term)
    );
  }),
);

const pagination = usePagination(filteredCategories, 10);

watch(searchTerm, () => pagination.resetPage());

async function confirmDelete(): Promise<void> {
  if (categoryToDelete.value) {
    isDeleting.value = true;
    try {
      await categoryStore.deleteCategory(categoryToDelete.value.id);
      categoryToDelete.value = null;
    } catch {
      // Keep the dialog open so the error message from the store is visible.
    } finally {
      isDeleting.value = false;
    }
  }
}

function cancelDelete(): void {
  categoryToDelete.value = null;
  categoryStore.deleteError = null;
}
</script>

<template>
  <div class="mx-auto max-w-7xl">
    <div class="mb-6 flex flex-wrap items-center justify-between gap-4">
      <h1 class="text-2xl font-bold text-gray-800">{{ t('inventory.categories.title') }}</h1>
      <RouterLink
        to="/inventory/categories/create"
        class="rounded-lg bg-brand px-4 py-2 text-sm font-semibold text-white transition hover:bg-brand-hover"
      >
        {{ t('inventory.categories.newCategory') }}
      </RouterLink>
    </div>

    <div class="mb-4">
      <SearchInput id="categories-search" v-model="searchTerm" />
    </div>

    <p v-if="categoryStore.isLoading" class="py-10 text-center text-sm text-gray-400">
      {{ t('inventory.categories.loading') }}
    </p>
    <p v-else-if="categoryStore.error" class="py-10 text-center text-sm text-red-500">
      {{ t(categoryStore.error) }}
    </p>
    <div v-else class="overflow-hidden rounded-xl border border-gray-200 bg-white shadow-sm">
      <div class="overflow-x-auto">
        <table class="w-full text-sm">
          <thead>
            <tr class="bg-gray-50 text-left text-xs uppercase tracking-wide text-gray-500">
              <th class="px-5 py-3.5 font-medium">{{ t('inventory.categories.form.codigo') }}</th>
              <th class="px-5 py-3.5 font-medium">{{ t('inventory.categories.form.name') }}</th>
              <th class="px-5 py-3.5 font-medium">{{ t('inventory.categories.form.description') }}</th>
              <th class="px-5 py-3.5 font-medium">{{ t('common.status') }}</th>
              <th class="px-5 py-3.5 text-right font-medium">{{ t('common.actions') }}</th>
            </tr>
          </thead>
          <tbody>
            <tr
              v-for="category in pagination.pageItems.value"
              :key="category.id"
              class="border-t border-gray-100 hover:bg-gray-50/60 transition"
            >
              <td class="px-5 py-3.5 font-mono text-gray-600">{{ category.codigo }}</td>
              <td class="px-5 py-3.5 font-semibold text-gray-800">{{ category.nombre }}</td>
              <td class="px-5 py-3.5 text-gray-600">{{ category.descripcion }}</td>
              <td class="px-5 py-3.5">
                <StatusChip
                  :label="category.activo ? t('common.active') : t('common.inactive')"
                  :tone="category.activo ? 'success' : 'neutral'"
                />
              </td>
              <td class="px-5 py-3.5 text-right">
                <div class="flex justify-end gap-2">
                  <RouterLink
                    :to="`/inventory/categories/${category.id}/edit`"
                    class="rounded-md px-2 py-1 text-xs font-medium text-brand transition hover:bg-brand/10"
                  >
                    {{ t('common.edit') }}
                  </RouterLink>
                  <button
                    type="button"
                    class="rounded-md px-2 py-1 text-xs font-medium text-red-500 transition hover:bg-red-50"
                    @click="categoryToDelete = category"
                  >
                    {{ t('common.delete') }}
                  </button>
                </div>
              </td>
            </tr>

            <tr v-if="pagination.totalItems.value === 0">
              <td colspan="5" class="px-5 py-10 text-center text-sm text-gray-400">
                {{ t('inventory.categories.empty') }}
              </td>
            </tr>
          </tbody>
        </table>
      </div>

      <PaginationBar
        :page="pagination.page.value"
        :total-pages="pagination.totalPages.value"
        :total-items="pagination.totalItems.value"
        :range-start="pagination.rangeStart.value"
        :range-end="pagination.rangeEnd.value"
        :page-size="pagination.pageSize.value"
        @update:page="pagination.page.value = $event"
        @update:pageSize="pagination.pageSize.value = $event"
      />
    </div>

    <ConfirmDialog
      :open="categoryToDelete !== null"
      :title="t('inventory.categories.deleteDialog.title')"
      :message="t('inventory.categories.deleteDialog.message', { name: categoryToDelete?.nombre ?? '' })"
      :confirm-label="t('common.delete')"
      :confirm-disabled="isDeleting"
      destructive
      @confirm="confirmDelete"
      @cancel="cancelDelete"
    >
      <p v-if="categoryStore.deleteError" class="mt-3 text-sm text-red-600">
        {{ categoryStore.deleteError }}
      </p>
    </ConfirmDialog>
  </div>
</template>

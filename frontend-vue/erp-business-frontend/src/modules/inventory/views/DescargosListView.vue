...
interface DescargoRealizadoRecord {
  id: string;
...
const items = ref<DescargoItemRow[]>([]);
...
const totalizarForm = reactive({
  clasificacion: 'MERMA / AVERÍA',
  fechaEmision: new Date().toISOString().split('T')[0],
  responsable: authStore.user?.nombreCompleto || 'ADMINISTRACIÓN',
  autorizadoPor: 'SUPERVISOR DE ALMACÉN',
  proposito: 'Egreso y desincorporación de stock de inventario',
  detalle: '',
});

const clasificacionesDisponibles = [
  'MERMA / AVERÍA',
  'DESINCORPORACION DE MATERIAL',
  'AJUSTE DE EXISTENCIA NEGATIVO',
  'USO INTERNO / CONSUMO',
...
const descargosRealizadosHistory = ref<DescargoRealizadoRecord[]>([]);
...
async function openReimprimirModal(): Promise<void> {
  ...
  try {
    ...
    const grouped = new Map<string, DescargoRealizadoRecord>();
    for (const row of data || []) {
      ...
      if (!grouped.has(docNum)) {
        grouped.set(docNum, {
          ...
          almacenId: String(row.depositoOrigenId || row.depositoDestinoId || ''),
          almacenNombre: row.depositoOrigenNombre || row.depositoDestinoNombre || 'Almacén',
          clasificacion: row.tipoClasificadoNombre || 'Descargo de Inventario',
          responsable: row.usuarioNombre || 'ADMINISTRACIÓN',
          autorizadoPor: 'SUPERVISOR DE ALMACÉN',
          proposito: row.motivo || 'Salida directa',
      ...
      rec.items.push({
        ...
        stockActual: 0,
        cantidad: Number(row.cantidadSalida || 0),
    ...
    descargosRealizadosHistory.value = Array.from(grouped.values());
  } catch (err) {
    console.error('Error cargando historial de descargos:', err);
    descargosRealizadosHistory.value = [];
  } finally {
  ...
  try {
    ...
    if (detalle.payload) {
      ...
      items.value = (detalle.payload.items || []).map((it: any) => {
        ...
        const stockActual = getProductStock(pId);
        return {
          productoId: pId,
...
onMounted(async () => {
  ...
  if (descargosRealizadosHistory.value.length === 0) {
    descargosRealizadosHistory.value = [
      {
        id: '1',
        documentoNumero: '00000845',
        fecha: new Date().toLocaleDateString('es-VE'),
        almacenId: warehouseStore.warehouseList[0]?.id ?? '1',
        almacenNombre: warehouseStore.warehouseList[0]?.name ?? 'ALMACEN GUAYANA',
        clasificacion: 'MERMA / AVERÍA',
        responsable: authStore.user?.nombreCompleto || 'ADMINISTRACIÓN',
        autorizadoPor: 'GERENCIA DE OPERACIONES',
        proposito: 'Desincorporación por merma física',
        detalle: 'Descargo formal de rollos fraccionados dañados durante traslado.',
        items: [
          {
            productoId: '1',
            codigo: 'VIN-001',
...
function onSelectProduct(id: string): void {
  ...
  formItem.stockActual = currentStock;
...
const isExceedingStock = computed<boolean>(() => {
  if (!formItem.productoId) return false;
  return formItem.cantidad > formItem.stockActual;
});

function incluirItem(): void {
  if (!formItem.productoId || formItem.cantidad <= 0 || isExceedingStock.value) return;
  ...
  if (existente) {
    ...
    items.value.push({
      ...
      stockActual: formItem.stockActual,
...
function openTotalizarModal(): void {
  ...
  totalizarForm.detalle = observacion.value || 'Egreso e incremento de salida de stock en almacén';
  showTotalizarModal.value = true;
}

// Execute Descargo Direct Movement and Open Print Preview
async function ejecutarDescargo(): Promise<void> {
  ...
  try {
    ...
    for (const item of items.value) {
      ...
      await stockStore.setQuantity(
        ...
        depositoId.value,
        newQty,
        `DESCARGO [${totalizarForm.clasificacion}]: ${totalizarForm.detalle.trim()}`,
      );
    }

    const record: DescargoRealizadoRecord = {
      id: String(Date.now()),
    ...
    descargosRealizadosHistory.value.unshift(record);

    successMessage.value = `¡Descargo #${nextDocNum} procesado exitosamente!`;
    ...
    errorMessage.value = resolveApiErrorMessage(err, 'Error ejecutando el descargo de inventario.');
  } finally {
...
async function guardarBorrador(): Promise<void> {
  ...
  try {
    await transformStore.guardarPreliminar('INV_DESCARGO', observacion.value.trim() || 'Borrador de Descargo', {
    ...
    successMessage.value = 'Borrador preliminar de Descargo guardado exitosamente.';
    resetForm();
  } catch (err) {
...
function reimprimirDescargoRecord(rec: DescargoRealizadoRecord): void {
  ...
  <div class="mx-auto max-w-[1400px] p-4 font-sans text-gray-800">
    ...
    <div class="mb-4 flex flex-wrap items-center justify-between gap-3 border-b border-gray-200 pb-3">
      <div class="flex items-center gap-3">
        ...
        <div>
          <h1 class="text-xl font-bold tracking-tight text-gray-900">Descargo</h1>
          <p class="text-xs text-gray-500">Módulo exclusivo de egresos y disminución de stock en almacén</p>
        </div>
      </div>

      <!-- Action Buttons Toolbar (Reimpresión, Cargar Preliminar) -->
    ...
    <div class="mb-5 grid gap-4 lg:grid-cols-3">
      ...
      <div class="lg:col-span-2 rounded-2xl border border-gray-200 bg-white p-5 shadow-sm space-y-4">
        <div class="flex flex-wrap items-center gap-4">
          ...
          <div class="flex-1">
            ...
            >
              <option value="" disabled>Seleccione el depósito / almacén de origen del descargo...</option>
              <option
        ...
        <div class="flex flex-wrap items-start gap-4">
          ...
          <div class="flex-1">
            <input
              v-model="observacion"
              type="text"
              placeholder="Ej. Salida por avería, desincorporación de material, merma o ajuste..."
      ...
      <div class="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm flex flex-col justify-between">
        <div class="flex items-center justify-between border-b border-gray-100 pb-2">
          ...
          <span class="font-mono text-sm font-bold text-blue-700">NUEVO DESCARGO</span>
        </div>
        ...
        <div class="flex items-center justify-between border-b border-gray-100 py-2 text-xs">
          ...
          <span class="font-mono font-bold text-blue-800">${{ totalCosto.toFixed(2) }}</span>
        </div>
    ...
    <div class="mb-5 rounded-2xl border border-gray-200 bg-white p-4 shadow-sm">
      <h3 class="mb-3 text-xs font-bold uppercase tracking-wider text-gray-700 flex items-center gap-2">
        <span>➖ Insertar Producto al Descargo</span>
      </h3>
      <div class="grid gap-3 sm:grid-cols-12 items-end">
        ...
        <div class="sm:col-span-2">
          <label class="mb-1 block text-xs font-medium text-gray-600">Ctd. a Egresar:</label>
          <div class="flex items-center gap-1">
            <input
              ...
              step="0.0001"
              min="0.0001"
              class="w-full rounded-lg border px-2.5 py-1.5 text-xs font-mono font-bold outline-none focus:border-brand"
              :class="isExceedingStock ? 'border-red-400 bg-red-50 text-red-800' : 'border-gray-300'"
            />
          </div>
        </div>
        ...
        <div class="sm:col-span-1 flex justify-end">
          <button
            type="button"
            :disabled="!formItem.productoId || formItem.cantidad <= 0 || isExceedingStock"
            class="w-full rounded-lg bg-blue-600 px-3 py-2 text-xs font-bold text-white shadow transition hover:bg-blue-700 disabled:opacity-40"
            @click="incluirItem"
          >
            Insertar
          </button>
      ...
      <p v-if="isExceedingStock" class="mt-2 text-xs font-semibold text-red-600">
        ⚠️ No puede egresar una cantidad ({{ formItem.cantidad }}) superior a la existencia disponible ({{ formItem.stockActual }}).
      </p>
    </div>

    <!-- Tabla Principal de Ítems Egresados -->
    <div class="mb-4 rounded-2xl border border-gray-200 bg-white overflow-hidden shadow-sm">
        ...
        <table class="w-full text-left text-xs">
          <thead class="sticky top-0 bg-gray-100 text-gray-700 font-semibold shadow-xs">
            <tr>
              ...
              <th class="border-b px-3 py-2 text-right">Existencia</th>
              <th class="border-b px-3 py-2 text-right">Ctd. Egresada</th>
          ...
          <tbody class="divide-y divide-gray-100">
            ...
            >
              ...
              <td class="px-3 py-2 text-right font-mono text-gray-600">{{ item.stockActual }}</td>
              <td class="px-3 py-2 text-right font-mono font-bold text-blue-800">
            ...
            <tr v-if="items.length === 0">
              <td colspan="9" class="px-4 py-10 text-center text-xs text-gray-400">
                No hay productos añadidos al descargo. Utilice la barra de inserción superior.
              </td>
            </tr>
          </tbody>
        </table>
      ...
      <div class="flex flex-wrap items-center justify-between border-t border-gray-200 bg-gray-50 px-4 py-2 text-xs font-semibold text-gray-700">
        ...
        <div class="flex items-center gap-6 font-mono">
          ...
          <span>Total Costo: <strong class="text-blue-800 text-sm">${{ totalCosto.toFixed(2) }}</strong></span>
        </div>
      </div>
    </div>
    ...
    <div class="flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-gray-200 bg-white p-4 shadow-sm">
      ...
      <div class="flex items-center gap-3">
        ...
        <button
          ...
          class="rounded-lg border border-blue-600 bg-blue-50 px-4 py-2 text-xs font-bold text-blue-800 transition hover:bg-blue-100 disabled:opacity-50"
        ...
        <button
          ...
          class="inline-flex items-center gap-2 rounded-lg bg-blue-600 px-6 py-2 text-xs font-bold text-white shadow-md transition hover:bg-blue-700 disabled:opacity-50"
          @click="openTotalizarModal"
        >
          <span>☑ Totalizar Descargos</span>
        </button>
      </div>
    </div>

    <!-- Modal "Totalizar Descargos" (Basado exactamente en la imagen de referencia) -->
    <div
    ...
    >
      <div class="w-full max-w-md rounded-2xl bg-white p-6 shadow-2xl border border-gray-300">
        <div class="flex items-center justify-between border-b border-gray-200 pb-3 mb-4">
          <h2 class="text-base font-bold text-gray-900">Totalizar Descargos</h2>
          <button
            type="button"
        ...
        <div class="mt-6 flex justify-end gap-3 pt-2 border-t border-gray-100">
          ...
          <button
            ...
            class="inline-flex items-center gap-2 rounded-lg bg-blue-600 px-5 py-1.5 text-xs font-bold text-white shadow transition hover:bg-blue-700 disabled:opacity-50"
            @click="ejecutarDescargo"
          >
      ...
      <div class="w-full max-w-3xl rounded-2xl bg-white p-6 shadow-2xl border border-gray-200">
        <div class="flex items-center justify-between border-b border-gray-200 pb-3 mb-4">
          <h2 class="text-base font-bold text-gray-900 flex items-center gap-2">
            <span>🖨️ Historial y Reimpresión de Descargos Realizados</span>
          </h2>
          <button
            type="button"
            class="text-gray-400 hover:text-gray-600 font-bold"
        ...
        <div class="max-h-96 overflow-y-auto rounded-xl border border-gray-200">
          <table class="w-full text-left text-xs">
            ...
            <tbody class="divide-y divide-gray-100">
              <tr v-for="rec in descargosRealizadosHistory" :key="rec.id" class="hover:bg-blue-50/30">
                <td class="px-3 py-2 font-mono font-bold text-blue-800">#{{ rec.documentoNumero }}</td>
                ...
                <td class="px-3 py-2 text-center">
                  <button
                    type="button"
                    class="rounded bg-blue-600 px-3 py-1 text-xs font-bold text-white shadow hover:bg-blue-700"
                    @click="reimprimirDescargoRecord(rec)"
                  >
                    🖨️ Reimprimir
                  </button>
                </td>
              ...
              <tr v-if="isLoadingHistory">
                <td colspan="6" class="px-4 py-8 text-center text-xs text-gray-500">
                  <div class="flex items-center justify-center gap-2">
                    ...
                    <span>Consultando movimientos de descargo en la base de datos...</span>
                  </div>
                </td>
              </tr>
              <tr v-else-if="descargosRealizadosHistory.length === 0">
